...
- Version change: 1.0.0 → 1.1.0
  Rationale: MINOR bump. A new Core Principle (Article XV) is added without
  removing or redefining any existing principle; existing article ordering,
  Roman numerals and section grouping are preserved.
- Modified principles: none (no rename, redefinition, or removal).
- Added sections:
  • Core Principles
      – 第 XV 条 Top-Level Design Primacy（顶层设计优先）
        Constraints the speckit pipeline to first read the three artifacts
        under `harness/top_level_design/` before producing any spec/plan/
        tasks/implement deliverable.
- Removed sections: none.
- Source corrections applied while incorporating the new principle (the
  originating draft in `harness/prompt/constitution_add_top_level.md`
  contained typos that conflict with repository reality; documented here
  so the deviation is auditable):
  • "全部五份 artifact" → "全部三份 artifact". The directory
    `harness/top_level_design/` contains exactly three primary artifacts
    (workflow.md, architecture_modules.md, module_schemas.md); the
    `scripts/` subdirectory is tooling, not an artifact.
  • Numbering "1, 3, 4" in the artifact list → "1, 2, 3". Preserving the
    broken numbering would have left the list un‑navigable; the corrected
    ordering matches the reading sequence described in the principle body.
- Deferred / TODO items:
  • `harness/prompt/constitution_add_top_level.md` still carries the
      pre‑correction wording ("五份", "1, 3, 4"). It is a draft prompt
      artefact, not a constitution source; left unchanged. Future cleanup
      of the prompt file is out of scope for this amendment.
-->
# LangAgent 宪法（First Constitution）

> 本文件是 LangAgent 项目的最高级别约束文件。任何规范（spec）、方案（plan）、
> 任务（task）、代码改动在与之冲突时，均以本宪法为准。修改本宪法需走
> "第 XIV 条 宪法修订程序"。
...
`/speckit.analyze`、`/speckit.implement`）执行人在进入本阶段工作之前，**MUST**
先顺序读取 `harness/top_level_design/` 目录下的全部三份 artefact，并对该 feature
的设计主张与本宪法各条款的对齐点形成书面化的"对齐清单"，随后方可进入本阶段的
实际产出。

#### 1. 强制读取的顶层设计 artefact 清单（顺序即为读取顺序）

1. `harness/top_level_design/workflow.md` — Agent Loop 端到端业务流。
2. `harness/top_level_design/architecture_modules.md` — 模块拆分、依赖方向、
   CLI↔module 映射、T1/T2/T3 持久化布局、Tool Market、HITL 双轴、
   与 Constitution 的映射表。
3. `harness/top_level_design/module_schemas.md` — schema 对象的字段约束、
   JSON Schema literal、anti-corruption 规则、命名与版本策略。

#### 2. 约束细则

- **读取时机**：可在阶段开始时完成一次性加载；同一 feature 后续阶段可复用
  首次对齐结果。但若顶层设计发生变更（新增 / 删除 / 修订任一 artefact），
  **MUST** 重新执行对齐并刷新对齐清单。
- **对齐清单**：必须显式列出本 feature 的设计主张与宪法相关条款
  （至少覆盖第 I / II / IV / V / VI / VII / XII 条）的逐项对齐或偏离声明；
  任何偏离项 **MUST** 标注其 RFC 文件路径。
- **交付物引用**：spec、plan、tasks 三类交付物顶部 **MUST** 显式引用
  本条款，格式为 "已对齐第 XV 条 / 对齐清单见 `<path>`"；未引用者视为
  评审不通过。
- **不可绕过**：本条款不得以"敏捷 / 时间紧 / 实验性 feature"为由跳过；
  跳过即视为违反第 XIII 条 Hard No。
...
| 日期 | 版本 | 修订摘要 | 提案文件 |
| --- | --- | --- | --- |
| 2026-09-14 | v1.0.0 | 首版宪法：从模板占位符正式落地 14 条治理条款 | (init_constitution.md) |
| 2026-09-15 | v1.1.0 | 新增第 XV 条 Top-Level Design Primacy（顶层设计优先） | (constitution_add_top_level.md) |
