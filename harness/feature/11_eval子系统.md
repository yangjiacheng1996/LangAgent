...
> **Feature ID**: F11
> **批次**: **Phase 1 第 4 批**（task_loader + report_aggregator 无运行时依赖，可与 F05 并行）+ **Phase 2 第 6 批**（5 个 graders 与 F08/F09 并行）+ **Phase 3 第 8 批**（runner 整合 F10/F06/F07/F08/F09 完成后）
> **依赖**（**review.md v2.4.0 P1-3 修复后修订**）：
> - **模块 import 依赖**（runner.run() 内部实际 import；可通过 `architecture_modules.md` 依赖矩阵 21×21 边校验）：
>   - F06（`dir_loader.load()` 加载被评测智能体）
>   - **F07（RuntimeConfig 类型消费；**review.md v2.4.0 P0-2 修复**：F11 runner 不再内部调用 `config_resolver.resolve()`——RuntimeConfig 由 F10 dispatch 入口解析后通过 `config` keyword-only 参数注入；F11 内部仅消费 config 实例，不持有 F07 模块 import）**
>   - **F03（**review.md v2.2.0 P2-6 修复新增**：F11 runner.publish `eval_task_started` / `eval_task_done` 事件到 F03 `protocol_event_bus`；F03 §3.4 已登记这 2 种 event_type；F11 §3.1 eval.runner 行需 `from langagent.protocol.event_bus import publish`；F02 metrics_collector 订阅这 2 类事件记 latency）**
>   - F08（`main_loop_dispatcher.run_until_done()` 跑每条任务）
>   - F01（`chat_model_factory.create(config)` 用于 llm_judge judge_model 独立实例，避免 judge bias；config 由 F10 注入）
> - **运行时调用依赖**（接口契约，无 import，非 module_id 依赖矩阵边）：
>   - F10（`cli_runner.parse_argv()` 输出 dict 结构 `{'cli_args': dict, 'grader_only': str | None, 'task': str | None}` 通过 `runner.run(agent_dir, *, config, args)` 参数传入；F11 不反向 import F10 任何符号）
> - **非依赖**（澄清）：F09（runner.run() 不 import F09；F09 写盘由 F10 dispatch 在 eval 分支统一调 `exit_handler.cleanup()` 完成）
> **被依赖**: F12（打包后 `langagent eval` 二进制可独立运行）
> **状态**: 待启动 speckit.specify（review.md v2.1.0 修复后；本 feature prompt 已应用 v0.4.0 修复：M-NEW-1 runner.run() 返回 `EvalRunResult` 含 `(eval_report, final_state, exit_code)`；m-NEW-1 args 类型从 `CliArgs` 改为 `dict[str, Any]`；M-NEW-3 log tag rename `la.cli.eval.*` → `la.lifecycle.eval.*`；m-NEW-9 grader_extensibility 用途澄清；v1.1.0 修复：P0-3 批次按相位展开；P0-4 §三 依赖列补 F07 + §3.1 runner.run() 内部 RuntimeConfig 来源明确 + §四 TDD 新增 test_runner_resolves_config_from_args_cli_args_dict；P1-2 §3.2 exit_code 含义 1 款改写；P2-5 §四.2 补 grader runtime validator 兜底测试；P2-11 §3.3 tool_call_match 双层校验备注；v2.1.0 修复：P1-3 §3.1 runner 签名改为 keyword-only `config` 参数，F11 不再内部 resolve config；P2-4 §3.1 表格加 5 graders 不计入 19 module_id 注释）
...
2. 对每条 task 跑 1 次 F08 `run_until_done`，产出实际输出。
3. 用 5 种 grader 之一（`exact_match` / `contains` / `regex` / `llm_judge` / `tool_call_match`）判定 pass/fail。
4. 累计生成 `EvalReport`，写盘 + 通过 F10 输出。

本 feature 不引入独立的 runtime 模块，而是跨 F10 / F06 / F08 / F09 的端到端整合 + 新增 `langagent/eval/` 子包。
...
| `eval.graders.llm_judge` | `grade(expected: str \| list[str], actual: str, judge_model: BaseChatModel) -> bool` | ~150 | Phase 2（异步，与 F01 弱依赖） |
| `eval.runner` | `run(agent_dir: str, *, config: RuntimeConfig, args: dict[str, Any]) -> EvalRunResult`（**review.md v2.1.0 P1-3 修复后签名修订**：`config` 改为 keyword-only 强制传入；F11 runner 不再 import / 调用 `config_resolver.resolve()`——RuntimeConfig 由 F10 dispatch 入口解析后注入；**args dict schema**：`{'cli_args': dict[str, Any]（cli_runner.parse_argv() 输出）, 'grader_only': str \| None, 'task': str \| None}`；**review.md v0.4.0 m-NEW-1 修复后**：`args` 类型从 `CliArgs` 改为 `dict[str, Any]`，避免 F11 跨层 import F10 的 `CliArgs` 类型；入口发射 `la.lifecycle.eval.start`；出口 EvalReport 写入磁盘前发射 `la.lifecycle.eval.summary`；**review.md v0.4.0 M-NEW-1 修复后**返回 `EvalRunResult(eval_report, final_state, exit_code)` 而非 `int`，由 F10 dispatch 调 `exit_handler.cleanup(result.final_state, config, eval_report=result.eval_report)`；`exit_code` 字段在 runner 内部通过 `worst_of(all_failure_exit_codes)` 仲裁计算，不直接由 dispatch 返回）+ `_run_one_task(spec, agent, graph, config) -> TaskResult`（收尾发射 `la.lifecycle.eval.case_done`；**review.md v2.2.0 P2-6 修复新增**：每条 task 开始 / 完成时 publish `eval_task_started` / `eval_task_done` 事件到 F03 `protocol_event_bus`；F02 metrics_collector 订阅这 2 类事件记 task 级 latency + token 用量；F11 runner 通过 `from langagent.protocol.event_bus import publish` 调用） | ~280 | Phase 3 |
...
F11 体量 ~900 行实现 + ~50 测试用例，比 F06/F07/F08 大 4-6 倍；为降低 PR 风险，分 3 个相位开发：

- **Phase 1**（首批 PR，独立）：`eval/task_loader.py` + `eval/report_aggregator.py`。两者无运行时依赖，可与 F02 / F05 / F03 并行开发。
...
- **`EvalRunResult`**（**review.md v0.4.0 M-NEW-1 修复新增**：frozen dataclass）：
  ...
  from langagent.runtime.agent_state import AgentState  # F08 拥有；F11 仅类型注解引用

  @dataclass(frozen=True)
  class EvalRunResult:
      """`eval.runner.run()` 返回值；review.md v0.4.0 M-NEW-1 修复明确。"""
      eval_report: EvalReport                    # F11 跑完所有 task 后聚合的报告
      final_state: AgentState                    # 最后一条 task 跑完后的 final state；F10 传给 F09 cleanup()
  ...
  >
  > `exit_code: int = worst_of([all_task_exit_codes + 0])`，由 F11 `eval_runner` 内部计算。F11 runner 内部**不发** `la.cross_cutting.metrics.emit` 日志（该 tag 由 F02 metrics_collector 拥有）；F11 runner 在 task 失败时改发 `la.cross_cutting.event_handler_error` 日志（**review.md v1.1.0 P1-2 修复改写**），payload 含 `task_id` / `error_type` / `error_message`；`exit_code != 0` 时 F11 runner 记录到 `runner._failed_task_count` 内部计数器。该字段 3 个用途（不重叠）：
  ...
  > 3. **F10 dispatch 不直接返回此字段**（统一走 `exit_handler.cleanup()` worst_of() 路径返回最终退出码）：F10 §3.4 eval dispatch 调 `result = eval_runner.run(agent_dir, args)` 后调 `exit_handler.cleanup(result.final_state, config, eval_report=result.eval_report)`，`result.exit_code` 字段被忽略（最终退出码由 cleanup 自身聚合决定；与 v0.5.0 §二.A.1 + v2.1.0 P1-3 修复后统一路径一致）。
  >
...
- [ ] `test_run_one_task_converges`：mock F08 run_until_done 返回 AIMessage("Hello, Alice!") + grader=exact_match + expected="Hello, Alice!" → pass。
...
- [ ] `test_run_accepts_dict_args_not_cli_args`（**v0.4.0 m-NEW-1 修复新增**）：`run(agent_dir, {"grader_only": "llm_judge", "task": "task_001"})` 正常运行；不依赖 F10 CliArgs 类型。
...
F11 在 eval 阶段发射 3 个 `la.lifecycle.eval.*` tag（详见 §3.1 eval.runner）；F11 通过 `from langagent.cross_cutting.logger import emit` 调用 F02 logger 接口。
...
- [ ] `test_eval_runner_log_tags_in_whitelist`：上述 3 个 tag 全部在 F02 `ALLOWED_TAGS` 集合（≥**46** 项）中。
...
7. **依赖分两类**（**review.md v2.4.0 P1-2 修复明确**）：
   - **模块 import 依赖**（F11 runner.run() 内部实际 import）：F06（`dir_loader.load()`）+ F07（**仅类型注解引用** RuntimeConfig）+ F08（`main_loop_dispatcher.run_until_done()`）+ F03（`event_bus.publish`）+ F02（`logger.emit`）+ F01（`chat_model_factory.create`）。
   - **运行时调用依赖**（通过 F10 dispatch 间接调用，F11 runner **不 import**）：F10（CLI 入口；F10 `parse_argv()` 输出 args dict 通过 `eval_runner.run(agent_dir, *, config, args)` 参数传入）+ F09（F10 dispatch eval 分支在 `eval_runner.run()` 返回后调 `exit_handler.cleanup(result.final_state, config, eval_report=...)` 写盘 + 退出码仲裁；F11 runner 不直接 import F09）。
   - F11 通过 `from langagent.cross_cutting.logger import emit` 调用 F02 logger 发射 3 个 eval 阶段 tag（`la.lifecycle.eval.{start,case_done,summary}`，**review.md v0.3.0 S-4 修复明确**）；F11 通过 `from langagent.protocol.event_bus import publish` 调用 F03 event_bus 在每条 task 开始 / 完成时 publish `eval_task_started` / `eval_task_done` 2 类事件（**review.md v2.2.0 P2-6 修复明确**：F03 §3.4 已登记这 2 种 event_type；F02 metrics_collector 订阅记 task 级 latency）。
...
- **F10**：`cli_runner.dispatch()` 对 `langagent eval` 调用 `eval_runner.run(agent_dir, args)`。
- **F06**：F11 调用 `dir_loader.load(agent_dir)` 加载被评测智能体。
- **F07**：F11 仅类型注解引用 `RuntimeConfig`（F07 拥有 schema 定义）；**review.md v2.1.0 P1-3 修复后**：F11 runner 不再 import / 调用 `config_resolver.resolve()`——RuntimeConfig 由 F10 dispatch 入口解析后通过 `config=` keyword-only 参数注入 runner。F07 仍是 RuntimeConfig schema 拥有方 + F10 dispatch 的依赖。
- **F08**：F11 对每条 EvalTaskSpec 调 `main_loop_dispatcher.run_until_done(graph, state, max_turns=...)`。
- **F09**：F11 跑完 eval 后由 F10 dispatch 调用 `exit_handler.cleanup(state, config, eval_report=report)` 写 EvalReport 到磁盘；F11 runner 不直接调 F09。
- **F02**：F02 metrics_collector 在每条 task 完成后产出 task 级 MetricsSnapshot。
- **F03**：F03 event_bus 在每条 task 开始 / 结束 publish `eval_task_started` / `eval_task_done` 事件。
- **F12**：F12 打包后，`./dist/langagent eval <agent-dir>` 可独立运行。