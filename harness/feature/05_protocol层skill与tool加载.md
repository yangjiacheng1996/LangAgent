...
- **side_effects 标注（review.md v0.4.0 m-NEW-2 修复后 fail-CLOSED 倾向）**：每个 tool 必须有 `SIDE_EFFECTS: list[str] = ["..."]` 模块级常量（由实现者声明）。**未声明 SIDE_EFFECTS** → F05 默认填 `[ToolSideEffect.NONE]`，并通过 `from langagent.cross_cutting.logger import emit` 发 `la.tool.suspicious_missing_side_effects` 警告日志（**review.md v0.1.0 R-001 修复后**：该 tag 是 tool 静态属性警告，**走 logger 路径不通过 event bus**；F02 `ALLOWED_TAGS` 集合 45 → 46 项；F04 guardrail 在拦截决策时对未声明 SIDE_EFFECTS 的 tool 触发"deny by default"——详见 F04 §三.4 决策矩阵）。
...
- [ ] `test_tool_side_effects_default_empty`：tool 无 `SIDE_EFFECTS` 常量 → `side_effects == [ToolSideEffect.NONE]`，且 F02 logger 收到 `la.tool.suspicious_missing_side_effects` 警告日志（**review.md v0.1.0 R-001 修复后**：通过 `cross_cutting_logger.emit()` 调 logger，不通过 event_bus.publish；**v0.4.0 m-NEW-2 修复后**：fail-OPEN 默认 + 警告日志；F04 guardrail 仍按 deny-by-default 处理未声明 side_effects 的 tool）。
...
- **F04**：F04 的 guardrail_middleware 读取 `ToolSpec.side_effects` + `ToolSpec.requires_approval` 字段做决策。
