# F05 — Protocol 层 Skill + Tool 加载（protocol_skill_loader + protocol_tool_registry）

> **Feature ID**: F05
> **批次**: 第 5 批（协议层）
> **依赖**: F03（event bus 发 `skill_loaded` / `tool_registered` 事件）/ F02（logger 记加载日志）/ F01（`BaseTool` / `AgentMiddleware` 类型注解）
> **被依赖**: F06（dir_load 调用 loader 扫描子目录）/ F08（main_loop 把 tool 注册到图）
...
- **`protocol_tool_registry`**：扫描 `tools/<tool_name>.py`，加载 `BaseTool` 子类，构造 `ToolSpec`。

Middleware 加载（`middleware/<name>.py` → `MiddlewareSpec` → `AgentMiddleware` 实例）由 F01 在 graph_compose 阶段处理（详见 F01 §3.4a）；F05 仅负责 skills + tools 两个子目录。`MiddlewareSpec` schema 在 module_schemas.md 已定义，由 **F01 拥有**（**评审 s-3 修复后**），F05 仅在 §3.2 引用，不重复实现。
...
3. `harness/top_level_design/module_schemas.md#schema-skill-spec-frontmatter` + `#schema-tool-spec` + `#schema-middleware-spec` —— schema 字段约束。
...
- `ToolSpec`（6 字段：`tool_id` / `tool_name` / `description` / `args_schema` JSON Schema / `enabled` + `requires_approval` 默认 False）
- `MiddlewareSpec`（**评审 s-3 修复后归 F01 拥有**：F05 仅在 §3.2 引用 schema 定义，不实现 loader；F01 §3.4a 实现完整加载协议；F05 deliverable 不再包含 MiddlewareSpec 的 Python 类型定义）
...
- **requires_approval 标注**：`REQUIRES_APPROVAL: bool = True/False` 模块级常量（默认 False）；这是唯一的工具级护栏标注。
...
- **sys.modules 清理策略（review.md v2.2.0 P2-4 修复新增）**：
  - F05 tool_loader 在 `register_all()` 返回前对每个成功加载的 tool 调 `sys.modules.pop(f"langagent_dynamic_tool_{tool_id}", None)`（立即释放）；失败 tool 不注册不需清理。
  - F05 skill_loader 同理：`sys.modules.pop(f"langagent_dynamic_skill_{skill_name}", None)`。
  - 进程退出前由 F09 `exit_handler.cleanup()` 兜底扫描：清空 `sys.modules` 中所有前缀 `langagent_dynamic_` 的 entry（防止中途异常导致泄漏）；兜底清理放在 §3.3 关闭顺序 step 4（`checkpoint_adapter.close`）之后、step 5（`metrics_collector.snapshot`）之前执行。
  - F01 middleware_loader 在 `state_graph_builder.build()` 返回前清理（详见 F01 §3.4a）：`sys.modules.pop(f"langagent_dynamic_middleware_{name}", None)`；命名空间前缀 `langagent_dynamic_middleware_` 与 F05 不同，**避免 tool / middleware 同名时冲突**。
  - 测试覆盖：F05 §四.2 `test_tool_dynamic_import_no_sys_path_pollution` 已存在；新增 `test_tool_loader_cleans_sys_modules_on_register` + `test_tool_loader_cleans_sys_modules_on_skill_load_failed`。
- **tool_id 唯一性（review.md v0.4.0 m-NEW-10 修复后异常传播路径明确）**：重复 `tool_id` 抛 `ToolIdDuplicateError`，**不包装为 F06 `AgentDirInvalidLayoutError`**，**不归类为目录布局错误**。F06 调用 `protocol_tool_registry.register_all()` 时捕获 `ToolIdDuplicateError` → 重新抛出（不包装），退出码 70 由 F10 dispatch 捕获并返回。

### 3.5 MCP 连接器（**review.md M-2 修复后**）

> **v1 不实现 MCP**；宪法第 V 条 1 款强制布局 + 顶层设计三件均无 `.langagent/mcp.yaml` / `connectors/` 目录约定。F05 仅加载 `tools/<tool_name>.py` 与 `skills/<skill_name>/SKILL.md` 两种内置来源。v2+ feature 加 MCP 客户端时，再扩展 `register_all(extra_dirs=...)` 接口。
...
- [ ] `test_tool_id_duplicate`：两个 tool 文件同 tool_id → 抛 `ToolIdDuplicateError`，退出码 70（**v0.4.0 m-NEW-10 修复后**：异常不包装为 F06 `AgentDirInvalidLayoutError`；F06 register_all() 直接重新抛出）。
...
- [ ] `test_tool_registry_handles_import_error_gracefully`：构造 1 个有 Python 语法错误的 tool → 跳过 + 发 `tool_load_failed` 事件（待与 F03 协调新事件类型）。

## 五、关键约束

1. **dynamic import 不污染 sys.path**：必须用 `importlib.util.spec_from_file_location()`。
2. **版本 semver 严格校验**：`^v\d+\.\d+\.\d+$` 正则，缺位 / 多位 / 非数字字符全报错。
3. **fail-soft 加载**：单个 skill / tool 加载失败不阻塞其它加载；发 `*_load_failed` 事件即可。
4. **不依赖 LangSmith**：skill / tool 不读 LangSmith；MCP 预留位**v1 不实现**（review.md M-2 修复）。
5. **依赖方向**：protocol 层不依赖 runtime / cli / primitives（仅 cross_cutting_logger + protocol_event_bus）。
6. **TDD 刚性**：先 Red 后 Green 再 Refactor；测试 fixture 提供 `tests/fixtures/sample_agent/skills/` + `tools/`。
7. **第 XV 条对齐清单**：与宪法第 V 条（智能体目录契约）/ 第 VII 条（Middleware 与工具规则）/ 第 XIII 条逐项对齐。

## 六、本 feature 不包含

- 不实现 middleware 加载（→ F01；F01 在 graph_compose 阶段读 `middleware/<name>.py`）。
- 不实现 LoadedAgent 构造（→ F06）。
- **不实现 MCP 客户端**（v1 不实现；review.md M-2 修复后整个 MCP 子节移除）。
- 不实现 channel / identity / memory / sandbox 加载（→ F13）。
...
- [ ] `langagent/protocol/skill_schemas.py` + `tool_schemas.py`（**review.md v1.0.0 P2-4 修复**：原 `schemas.py` 单文件拆分为两个文件，按"skill vs tool"职责分离）：
  ...
  - `langagent/protocol/tool_schemas.py`：`ToolSpec` 1 个 Pydantic / dataclass 类型。
  - **不导出 `MiddlewareSpec`**（review.md s-3 修复后归 F01）。
- [ ] `tests/protocol/test_skill_loader.py` + `test_tool_registry.py`：≥17 个测试用例（移除 3 个 side_effects 相关测试）。
...
- **F06**：`runtime_dir_loader.load(agent_dir)` 调用 `protocol_skill_loader.load_all(skills_dir)` 与 `protocol_tool_registry.register_all(tools_dir)`；产出 `LoadedAgent.tool_ids` + `skill_names`。
- **F08**：F08 在 main_loop 中通过 `tool_registry.get_by_id(tool_id)` 获取 `BaseTool` 实例列表，注入到 LangGraph 图。
- **F04**：F04 的 guardrail_middleware 读取 `ToolSpec.requires_approval` 字段做决策。
- **F01**：F01 在 graph_compose 阶段读 `middleware/<name>.py` 并实现 MiddlewareSpec loader（**评审 s-3 修复后**：F01 是 MiddlewareSpec 的实际拥有者；F05 仅引用 schema）。