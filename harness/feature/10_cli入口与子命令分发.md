# F10 — CLI 入口与子命令分发（init / run / eval / doctor）

> **Feature ID**: F10
> **批次**: 第 8 批（CLI 层）
> **依赖**: F06（dir_loader.load / write_template）/ F07（config_resolver.resolve）/ F08（main_loop_dispatcher.dispatch）/ F09（exit_handler.cleanup + run_doctor_checks）/ F01（chat_model_factory.create + checkpoint_adapter.create + state_graph_builder.build + primitives_state_reducers reducer 注入，**v0.4.0 M-4 方案 C 修复后**）/**F11（仅 `langagent eval` 子命令懒导入 eval_runner.run，review.md v0.3.0 M-5 修复明确；v0.4.0 M-NEW-1 修复后返回 `EvalRunResult` 含 `(eval_report, final_state, exit_code)`）**
> **被依赖**: F11（eval 子系统通过 CLI 入口触发）/ F12（打包 CLI 二进制）
> **状态**: 待启动 speckit.specify（review.md v1.1.0 修复后；本 feature prompt 已应用 v0.4.0 修复：M-NEW-3 log tag rename `la.cli.*` → `la.lifecycle.*`；M-NEW-1 eval dispatch state 来源明确；v1.1.0 修复：P0-2 方案 B §3.4 run dispatch 删 compiled_graph 引用 + LoadedAgent 5 字段 + F10 独立持 CompiledStateGraph；P2-10 §四.2 第 6 项测试改写 dispatch_eval_returns_cleanup_exit_code）
...
- **第 I 条** 项目身份与边界：F10 是 LangAgent 对外暴露的"二进制命令"入口（与宪法第 I 条 2 款对齐）；
- **第 III 条** LangSmith 剥离：F10 CLI 严禁直接 `import langsmith` / 不读 LANGSMITH_* env；
- **第 XI 条** 打包与分发：F10 与 F12 协同——F10 的 `langagent/__main__.py` 是 PyInstaller 入口；
- **第 XIII 条** 禁止项：CLI × primitives 黑名单（F10 不直接 import `chat_model_factory` / `checkpoint_adapter`，避免破坏分层约束；probe 工厂由 F08 `build_doctor_probes()` 提供）。
...
> **review.md M-4 修复新增**：F10 dispatch 对 `langagent eval` 委托 `eval_runner.run(agent_dir, args)`（**F11 实现**）。`eval_runner` 是 F11 提供的跨 runtime/cli 整合模块，**不属于 17 个 module_id 计数**，由 F11 在 `langagent/eval/runner.py` 实现；F10 仅以 `from langagent.eval.runner import run as eval_runner_run` 形式调用。

### 3.2 Schema（本 feature 消费）

- `CliArgs`（frozen Pydantic）：`subcommand: str` + `agent_dir: str` + `cli_args: dict[str, Any]`（其余字段按子命令扩展）。
- `DoctorReport`（F09 拥有 schema 定义与 4 项 check 函数，本 feature 仅消费并格式化）。
...
> **review.md M-8 修复**：`--template` 参数 v1 不暴露；F06 §3.5 write_template() 不接收 template 参数；仅默认模板。v2 引入多模板时再加。
...
```
langagent init <name>:
    write_template(target_dir=./<name>, name=<name>)        # F06 提供（review.md M-8 修复：不接收 template 参数）
    # F06 §3.5 内部发射 la.lifecycle.init.start（review.md v2.1.0 P1-2 修复：init.end 不再由 F06 发射）
    print_template_summary(written_files)
    # review.md v2.1.0 P1-2 修复：init 子命令末尾调 exit_handler.cleanup() 走 exit_cleanup 阶段
    #   由 F09 在 step 10 发射 la.lifecycle.init.end（详见 F09 §3.3 init-only cleanup 模式）
    return exit_handler.cleanup(state=None, config=None, *, init_only=True)   # 阶段 6 exit_cleanup（init-only 分支）

langagent run [agent_dir]:
    loaded = dir_loader.load(agent_dir)                                  # 阶段 1 dir_load（F06 内部发射 la.runtime.dir_load.{start, ok, fail}）
    config = config_resolver.resolve(cli_args, agent_dir)                # 阶段 2 config_resolve（F07 内部发射 la.runtime.config_resolve.{start, priority_merge, ok, fail}）
    model = chat_model_factory.create(config)                            # 阶段 3 model_adapt（F01 内部发射 la.runtime.model_adapt.{start, endpoint_probe, ok, fail}）
    config = config.with_model(model)                                    # frozen 实例重建（review.md S-1 修复：用 with_model）
    checkpoint = checkpoint_adapter.create(config)                       # F01（review.md S-5 修复：先 create 再 build）
    graph = state_graph_builder.build(loaded, config, checkpoint)        # 阶段 4 graph_compose（F01 内部发射 la.runtime.graph_compose.{start, middleware_bind, tool_bind, ok, fail}）
                                                                        # review.md v1.1.0 P0-2 修复后：loaded 仅 5 字段（不含 compiled_graph），graph 由 F10 dispatch 独立持有
    state = AgentState(messages=[HumanMessage(input)], ...)
    final_state = main_loop_dispatcher.run_until_done(graph, state, max_turns)  # 阶段 5 main_loop（F08 内部发射 la.lifecycle.run.* 5 个 + la.runtime.main_loop.* 7 个共 12 个 tag；另 1 个 la.cross_cutting.guardrail.block 由 F04 发射，F08 不重发）
...
langagent doctor [agent_dir]:
    ...
    # review.md v1.0.0 P0-1 修复：F10 doctor dispatch 不再直接 import primitives
    ...
    #   改为调 F08 新增的 build_doctor_probes() 工厂（详见 F08 §3.6a）：
    ...
    checks = exit_handler.run_doctor_checks(
        config, loaded,
        model_probe_fn=lambda: probe_model_fn(config),       # review.md v1.0.0 P0-1 修复：callable 注入（probe 工厂由 F08 提供）
        checkpoint_probe_fn=lambda: probe_checkpoint_fn(config),
    )                                                                         # 4 项 check；F09 实现（F09 §3.7 接受 probe callable）
    ...
    # review.md v0.3.0 M-2 修复：F10 doctor dispatch 写出 DoctorReport 后 stdout 输出
...
按 `workflow.md` 退出码表实施；具体见 F09 exit_handler 退出码优先级。
...
| `langagent eval [agent-dir]` | `evals/` 目录不存在 | 66（**review.md v2.2.0 P2-1 修复**：与 `workflow.md#langagent-eval` §失败模式 `evals_dir_missing` 对齐；EX_NOINPUT 语义一致性优于 v1.0.0 提出的"0 tasks 非阻塞"设计意图——后者与 POSIX `EX_NOINPUT=66` 冲突。F11 runner 收到 `EvalsDirMissingError` 后立即返回 `EvalRunResult(eval_report=None, final_state=None, exit_code=66)`，由 F10 dispatch 走 `exit_handler.cleanup()` 统一退出码仲裁路径） |
| `langagent doctor [agent-dir]` | 4 项 check 全 ok / 仅 warn / **skipped** | 0（**review.md v2.2.0 P3-1 修复新增**：skipped 状态语义——若某 check 因 probe_fn 未注入（F09 §3.7 `test_doctor_check_model_without_probe_fn_returns_skipped`）返回 `status="skipped"`，F09 `run_doctor_checks()` 不将其计入 overall 失败项；overall 仍是 `ok` / `warn` / `error` 三选一，skipped 单项不影响退出码 0） |
...
| `langagent doctor` | `la.lifecycle.doctor.check` / `la.lifecycle.doctor.report` | F09 `runtime_exit_handler.run_doctor_checks()` / `cleanup()` |

> F10 dispatch **不调用** `cross_cutting_logger.emit()`；F10 仅通过 stdout 输出对话内容（review.md v0.3.0 M-2 修复）、格式化报告（review.md v0.3.0 M-2 修复）、编排 probe 函数（**review.md v1.0.0 P0-1 修复**：probe 工厂由 F08 `build_doctor_probes()` 提供；F10 不再直接 import `chat_model_factory` / `checkpoint_adapter`，避免违反 `architecture_modules.md#mod-cli-runner` 的 CLI × primitives 黑名单）。所有 `la.*` tag 由 F06 / F07 / F08 / F09 / F03 / F02 / F04 / F01 / F11 9 个 feature 发射；F02 logger 提供 `emit()` 接口与 ≥**46** 项白名单校验（详见 F02 prompt §3.3）。
...
- [ ] `test_dispatch_run_prints_chat_to_stdout`：构造 LoadedAgent + mock F08 run_until_done → F10 dispatch run 后 capture stdout 含 `[assistant] ...`。
- [ ] `test_dispatch_eval_prints_report_to_stdout`：mock F11 eval_runner.run 返回 EvalReport → F10 dispatch eval 后 stdout 含通过率。
- [ ] `test_dispatch_doctor_prints_report_to_stdout`：构造 DoctorReport → F10 dispatch doctor 后 stdout 含 4 项 check 状态。
...
3. **CLI 严禁直接发射 `la.*` tag**（review.md v0.3.0 S-2 修复明确，architecture_modules.md#mod-cli-parser 硬约束）：F10 dispatch **不得 import `langagent.cross_cutting.logger`**；所有 13 个 cli.* tag 由 runtime / protocol / cross_cutting 各 feature 发射，F10 仅作为入口编排。
4. **依赖方向**：cli → runtime → protocol / cross_cutting / primitives；cli 不得反向依赖 primitives 内部细节（仅通过 runtime API 间接调用）。
5. **stderr = 日志 / stdout = 用户输出**：语义分离（workflow.md run 命令约定）；F10 dispatch 用 `print(format_chat_message(...), flush=True)` 等写入 stdout，用 cli_output_formatter 系列 API 格式化（review.md v0.3.0 M-2 修复明确）。
...
10. **doctor 编排注入 probe 函数**（review.md v0.3.0 第四批 S-5 修复 + **v1.0.0 P0-1 修复**）：F10 doctor dispatch **通过 F08 `build_doctor_probes()` 工厂**取得 `probe_model_fn` / `probe_checkpoint_fn`，把两个 probe（`lambda: probe_model_fn(config)` / `lambda: probe_checkpoint_fn(config)`）注入到 F09 `run_doctor_checks(*, model_probe_fn=, checkpoint_probe_fn=)`；**F10 自身不直接 import `langagent.primitives.chat_model_factory` 或 `langagent.primitives.checkpoint_adapter`**（CLI × primitives 黑名单，architecture_modules.md#mod-cli-runner 硬约束）；F09 不直接 import primitives（保留 S-5 修复成果）。F08 是 probe 工厂的天然归属——runtime 层依赖 primitives 是 matrix 允许的，且 F08 已是 primitives 层的消费方（通过 `primitives_state_graph_builder.build()`）。
...
- 不实现模型实例化（→ F01）。
- 不实现 checkpointer 实例化（→ F01）。
- 不实现 event_bus / logger / audit（→ F03 / F02 / F04，本 feature 仅调用其 API）。
...
- **F06**：F10 在 init / run / eval / doctor 中调用 `dir_loader.write_template` 或 `dir_loader.load`；F06 内部发射 `la.lifecycle.init.start` 与 `la.runtime.dir_load.{start,ok,fail}`（F10 不发射；v0.4.0 M-NEW-3 rename；**review.md v2.1.0 P1-2 修复**：F06 不再发射 `la.lifecycle.init.end`，该 tag 改由 F09 `cleanup()` 在 init 子命令末尾的 exit_cleanup 阶段统一发射）。
- **F07**：F10 在 run / eval / doctor 中调用 `config_resolver.resolve(cli_args, agent_dir)`；F07 内部发射 `la.runtime.config_resolve.{start,priority_merge,ok,fail}`。
- **F08**：F10 在 run 中调用 `main_loop_dispatcher.run_until_done(graph, state)`；F08 内部发射 `la.lifecycle.run.*` 5 个 + `la.runtime.main_loop.*` 7 个 tag；F10 run dispatch 把 `final_state["messages"]` 写入 stdout（review.md v0.3.0 M-2 修复；v0.4.0 M-NEW-3 rename；v0.5.0 §三.B.1 修正 lifecycle.run.* 5 个而非 6 个）。
- **F09**：F10 在所有子命令结束后调用 `exit_handler.cleanup(state, config, *, doctor_report=, eval_report=, init_only=False)`；F09 内部发射 `la.runtime.exit_cleanup.{start,checkpointer_close,report_write,audit_flush,ok,fail}` + init-only 分支发 `la.lifecycle.init.end`（**review.md v2.1.0 P1-2 修复新增**）；F10 doctor 编排 `probe_model` / `probe_checkpoint` lambda 注入到 F09 `run_doctor_checks()`（review.md v0.3.0 第四批 S-5 修复）；F10 init dispatch 末尾传 `init_only=True` 走极简分支。
- **F01**：F10 直接调用 `chat_model_factory.create()` + `checkpoint_adapter.create()` + `state_graph_builder.build()`（review.md v0.3.0 S-5 修复：F10 dispatch 显式调用三步）；F01 内部发射 9 个 model_adapt / graph_compose tag（review.md v0.3.0 S-1 修复明确）。
- **F11**：F10 在 eval 中委托 `eval_runner.run(agent_dir, *, config, args)`（**review.md v2.1.0 P1-3 修复**：F10 dispatch 入口 resolve config 后通过 keyword-only `config` 参数注入 runner；F11 runner 不再内部 resolve）；F11 内部发射 `la.lifecycle.eval.{start,case_done,summary}`（review.md v0.3.0 S-4 修复；v0.4.0 M-NEW-3 rename）；F10 dispatch eval 后 stdout 输出 EvalReport（review.md v0.3.0 M-2 修复；v0.4.0 M-NEW-1 修复后返回 `EvalRunResult` 含 `(eval_report, final_state, exit_code)`；v0.5.0 字段名统一为 `final_state`；F10 §3.4 eval dispatch 走 `exit_handler.cleanup()` 统一路径，与 `langagent run` dispatch 同构）。